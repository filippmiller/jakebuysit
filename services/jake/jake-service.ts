...
export const jakeService = new JakeService();
  ...
  /**
   * Get research animation choreography
   * Returns 3-stage voice + animation coordination
   */
  async getResearchChoreography(offerData: OfferData) {
    const { getResearchChoreography } = await import('./research/index.js');
    return getResearchChoreography(offerData);
  }
}
